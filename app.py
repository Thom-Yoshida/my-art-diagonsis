...
def create_pdf(json_data):
    ...
    # ★ P5-P8: Space Adjustments & 12% Margin Enforcement ★
    ...
    # 完全に12%余白の内側に収める幅設定
    # HeaderとBodyの間隔を 12mm -> 16mm に拡大して被り防止
    for i, a in enumerate(archs[:3]):
        ...
        c.setFillColor(HexColor(COLORS['pdf_text']))
        # 幅制限: 全幅 - 左インデント(8mm)
        draw_wrapped_text(c, a.get('detail', ''), MARGIN_X + 8*mm, y - 16*mm, FONT_SANS, 14, CONTENT_WIDTH - 15*mm, 20)
        y -= 48*mm
    c.showPage()
    ...
    for i, step in enumerate(steps):
        ...
        c.setFillColor(HexColor(COLORS['pdf_sub']))
        # 間隔を 12mm -> 15mm に拡大
        draw_wrapped_text(c, step.get('detail', ''), MARGIN_X + 30*mm, y - 15*mm, FONT_SANS, 12, CONTENT_WIDTH - 40*mm, 18)
        y -= 45*mm
    c.showPage()
    ...
    # 中央配置・12%マージン厳守
    draw_wrapped_text(c, q_text, width/2, height/2 + 20*mm, FONT_SERIF, 28, CONTENT_WIDTH, 36, centered=True)
    c.setFont(FONT_SANS, 18)
